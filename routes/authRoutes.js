...
router.post('/registro', upload.single('foto'), handleUploadError, async (req, res) => {
  try {
    ...
    if (!nombre || !email || !password) {
      return res.status(400).json({ error: 'Nom, email i contrasenya requerits' });
    }
    ...
    res.status(201).json({ token, usuari: { id: usuari._id, nombre: usuari.nombre, email: usuari.email, rol: usuari.rol, foto: usuari.foto } });
  } catch (err) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({ token, usuari: { id: usuari._id, nombre: usuari.nombre, email: usuari.email, rol: usuari.rol, foto: usuari.foto } });
  } catch (err) {
...
router.get('/perfil', protegir, async (req, res) => {
  ...
  res.json({ id: usuari._id, nombre: usuari.nombre, email: usuari.email, rol: usuari.rol, foto: usuari.foto });
});
...
router.put('/perfil', protegir, upload.single('foto'), handleUploadError, async (req, res) => {
  try {
